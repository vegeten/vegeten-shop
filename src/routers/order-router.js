...
orderRouter.post('/orders', loginRequired, async (req, res, next) => {
  try {
    const userId = req.currentUserId;
    const { email, phoneNumber, address, price, products } = req.body;
    console.log(userId, email, phoneNumber, address, price, products);
...
// 유저가 해당 주문 내역 삭제
orderRouter.delete('/orders', loginRequired, async function (req, res, next) {
  try {
...
// 유저가 해당 주문 내역 삭제
orderRouter.delete('/orders/:ordersId', loginRequired, adminAuth, async function (req, res, next) {
  try {
    const { orderId } = req.params;
    // 특정 id에 맞는 사용자 정보를 얻음
    const deleteOrder = await orderService.deleteOrder(orderId);
